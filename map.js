...
var directionsButton = document.getElementById('button');
...
async function directions() {
    document.getElementById("outer").innerHTML = "<span class='loader' id='loader'><span class='loader-inner'></span></span>";
    let options = {};
    let origin = document.getElementById("start").value;
    let destination = document.getElementById("end").value;
    if (origin == "" || destination == "") {
        textbox.innerHTML = "Please enter both an Origin and Destination."
    }
    else {
    ...
    await sleep(3500); //Loading
    options = await textToTime(options);
    bestOption = await processResults(options);
    if (bestOption != "") {
        if (bestOption == "WALKING") {
            document.getElementById("img").innerHTML = "<img src='walk.png'>";
        }
        if (bestOption == "BICYCLING") {
            document.getElementById("img").innerHTML = "<img src='bike.jpg'>";
        }
        if (bestOption == "TRANSIT") {
            document.getElementById("img").innerHTML = "<img src='train.png'>";
        }
        if (bestOption == "DRIVING") {
            document.getElementById("img").innerHTML = "<img src='car.png'>";
        }
        let request = {
            origin: origin,
            destination: destination,
    ...
    document.getElementById("outer").innerHTML = "<div class='button_slide slide_right' id='button' onclick='directions()'> Get Directions </div>";
}
